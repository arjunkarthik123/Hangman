...
while wrong<trials:
    guess = input("Enter your guess for a letter in the word: ")
    for i in word:
        #print(i)
        if guess == i:
            empty[y] = i
           # print(empty)
            count = count + 1
            y = y + 1
            x = ' '.join([str(item) for item in empty])
            print(x)
        else:
            y = y+1
    if check == empty:
        ...
        break
    if count == 0:
        print("This letter is wrong, try again.")
        wrong = wrong+1
    count = 0
    y = 0

if final_check == False:
    print("Sorry! You didn't guess the word, the word is: ",word)
